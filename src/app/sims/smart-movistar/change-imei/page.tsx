...
export default function Page() {
  const columns = {
    "": [
      ...
      { label: "Envio de alertas", key: "imei_change_conf", width: 300 },
  ...
  useEffect(() => {
    if (data.length) {
      let parseData = data.map((el: any) => ({
        ...
        imei_change_conf: el.imei_change_conf === "T" ? "Si" : "No",
        config:
          el.imei_change_conf === 1
            ? "La SIM se desactiva y permanece desactivada hasta una activación manual"
            : el.imei_change_conf === 2
  ...
  const cofiguration = [
    ...
    { value: 2, label: "La SIM no se desactiva" },
  ];
  ...
  const handleSubmit = async (e: any) => {
    ...
    setLoading(true);
    await axiosInstance.put("/imei/updateSims", values);
    setSelected(false);
    setSelectedConfig(1);
    setSelectedSims([]);
    setRestSims([]);
  ...
  return !loading ? (
    <div className="pr-5 overflow-y-auto h-[85vh] border-solid border-2 border-[#d6d6d6] rounded-[32px] p-5 mr-5">
      <div className="flex gap-20">
        <h2 className="text-[#24A2CE] text-2xl">Configurar para cambio de IMEI</h2>
      ...
      <div className="mt-5">
        <p className="mb-5">Configuración en caso de sobre cambio de IMEI</p>
        ...
        <div className="flex items-center mt-2">
          ...
          <p>Notificar vía correo electrónico alertas para cambio de IMEI</p>
          {selected && (
            <TextField
